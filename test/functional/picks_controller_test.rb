...
class PicksControllerTest < ActionController::TestCase
  ...
  def test_index_redirects_when_picks_editable_and_player_not_looking_at_own_picks_and_not_logged_in_as_admin
    AdminConfig.make(:picks_editable => true)
    login_as Player.make
  ...
  test 'edit with multiple years' do
    Blueprints.announce_nominations(Date.today.year - 1)
    Blueprints.announce_nominations(Date.today.year)
    player = Player.make
    login_as player
    get :edit, :player_id => player.to_param
    assert_equal Category.for_year(Date.today.year).count, assigns(:categories).size
  end
